...
`shared/design-system/token-taxonomy.md` for token category and naming rules,
and `shared/design-system/component-anatomy-reference.md` for component slots,
required parts, accessibility hooks, and state hooks. Use
`shared/design-system/component-state-guidelines.md` for state coverage and
handoff boundaries between design-system seeds and prototype configs. Use
`shared/design-system/accessibility-token-guidelines.md` for contrast, focus,
target size, readable type, reduced motion, disabled legibility, opacity, and
non-color state checks. Use
`shared/design-system/responsive-system-guidelines.md` for responsive token,
component, density, sticky region, and prototype handoff expectations. Use
`shared/design-system/visual-style-calibration.md` when style references, brand
...
`shared/design-system/design-system-handoff-checklist.md` before a seed is
...
`shared/design-system/anti-generic-ui-guidelines.md` when schema-valid output
...
| Token naming | `palette.colors`, `typography.fontFamilies`, `typography.scale`, component token references | Tokens follow `shared/design-system/token-taxonomy.md`, use semantic dot notation, and describe intent rather than raw value names. | Tokens are value-named, duplicated, orphaned, or too implementation-specific. |
...
| States | `components.*.states`, `components.*.rules`, `usageNotes` | Components that users act on follow `shared/design-system/component-state-guidelines.md` and include default, focus, disabled, loading, error, or success states as relevant. | Interactive components have only default styling or omit focus/disabled/error behavior. |
| Accessibility | `palette.accessibilityNotes`, `components.*.rules`, `usageNotes`, `openQuestions` | Token accessibility follows `shared/design-system/accessibility-token-guidelines.md`; contrast, focus visibility, text legibility, target size, icon labeling, disabled legibility, non-color state, and reduced-motion risks are named. | Accessibility is isolated to generic notes or lacks actionable constraints. |
| Responsive behavior | `components.header.layout`, `components.header.height`, `components.footer.layout`, `components.cards.variants`, `usageNotes` | Responsive guidance follows `shared/design-system/responsive-system-guidelines.md`; priority explains what moves, collapses, hides, stacks, or stays persistent. | Responsive behavior is only "desktop/mobile" labels with no priority decision. |
...
1. Validate the seed against `shared/schemas/design-system-seed.schema.json`.
...
- Do not hand a seed to downstream app, blueprint, prototype, or implementation
  ...
  `shared/design-system/design-system-handoff-checklist.md`.
