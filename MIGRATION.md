...
## Sprint 010 `skills/` Source Mirror Removal

The committed top-level `skills/` source mirror has been removed. Skill source
now lives only in `plugins/individuals/`, and bundle composition lives in
`plugins/bundles/*/plugin.json`.

Runtime install targets still contain a `skills/` directory because agents load
flat skill folders from paths such as `.agents/skills` and `.claude/skills`.
Those runtime folders are generated by the installer, bundle builder, package
builder, or Codex plugin builder.

Use bundle-aware installs:
...
is not the supported path after this cleanup because it depends on repository
directory discovery rather than bundle manifests.

Older installed copies do not need in-place migration. Reinstall with the
bundle-aware installer to replace legacy skills with manifest-selected runtime
skills.
...
During the Sprint 001 transition:

- `skills/` remained the compatibility surface for the existing blueprint MVP.
