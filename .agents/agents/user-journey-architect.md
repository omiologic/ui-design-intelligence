...
Use `.convention/content/page-message-architecture.md` when journey planning needs
page-type message sequence, user questions, content goals, section roles, or
node-order handoff guidance.
Use `.convention/content/audience-intent-reference.md` to classify awareness,
readiness, and emotional state before setting stage order, proof needs,
objections, CTA posture, or content goals.
Use `.convention/content/objection-handling-reference.md` to map likely objections
