...
Use `.convention/content/content-pattern-library.md` to choose generic copy
...
`.convention/content/cta-patterns.md` to select CTA labels by user intent, journey
stage, action hierarchy, and interaction risk before applying tone variation.
Use `.convention/content/claim-risk-guidelines.md` to set `needsReview`, avoid
unsupported `productionReady` status, and rewrite unsupported claims into
review-safe draft language. Use
`.convention/content/proof-and-credibility-patterns.md` to select proof structures
that match the claim and available evidence without inventing testimonials,
stats, badges, guarantees, or case studies. Use
`.convention/content/page-message-architecture.md` to keep page-level copy roles
aligned to the page type, user intent, journey stage, and blueprint node order.
Use
`.convention/content/audience-intent-reference.md` to avoid copy that assumes the
wrong readiness, emotional state, proof need, CTA posture, or tone pressure.
Use
`.convention/content/objection-handling-reference.md` when page, section, CTA, FAQ,
...
Use
`.convention/content/content-accessibility-guidelines.md` to keep generated copy
plain, inclusive, label-clear, recovery-oriented, and understandable without
visual context. Use
`.convention/content/microcopy-guidelines.md` for labels, helper text, errors,
...
`.convention/content/laws-of-copywriting.md` as the shared strategy reference for
decision logic, benefit translation, story sequence, proof, objections, CTA
clarity, and action readiness. Use
`.convention/content/copy-quality-checklist.md` for pass/fail review across clarity,
...
- Shared references: `.convention/content/content-pattern-library.md`,
