...
- `shared/schemas/user-journey-map.schema.json`: audience goals, journey stages,
  user questions, needs, objections, and content goals.
- `shared/schemas/content-model.schema.json`: node-tied copy roles and review
  metadata.
- `shared/content/page-message-architecture.md`: page-level sequence by page
  type and user intent.
- `shared/content/cta-patterns.md`: CTA posture by intent, journey stage, and
  action risk.
- `shared/content/proof-and-credibility-patterns.md`: proof needs by claim and
  hesitation.
- `shared/content/microcopy-guidelines.md`: field, state, dialog, and recovery
  language.
- `shared/content/tone-of-voice/tone-of-voice-reference.md`: tone expression and
  restraint.
