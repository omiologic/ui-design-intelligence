...
```txt
study -> knowledge -> style reference -> design-system seed -> blueprint/wireframe -> prototype config -> later runtime
```
...
| 4 | `DesignSystemSeed` | Normalizes brand, palette, typography, iconography, component vocabulary, state names, layout rules, and accessibility constraints. | Blueprint, wireframe, and prototype skills. |
| 5 | `UIBlueprint` / `WireframeConfig` | Defines page, section, component, overlay, responsive, and structural node hierarchy. | Prototype config. |
| 6 | `PrototypeConfig` | Adds screens, routes, states, interaction flows, overlays, forms, navigation flows, and transitions over wireframe node references. | A future runtime, editor, preview player, or export engine. |
...
7. `generate-blueprint-from-study` or `generate-blueprint-from-knowledge`
8. `generate-prototype-from-blueprint` or `generate-prototype-from-knowledge`
9. `audit-prototype-flow`
10. `explain-prototype-interactions`
